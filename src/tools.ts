...
import { z } from "zod";
import { ld, ldList, out, fail, SLIM_FIELDS, DEFAULT_LIST_LIMIT, MAX_RESULTS_CAP } from "./api.js";
...
const PREMIUM_FIELDS = new Set([
  ...
  "ev_station_name", "ev_station_cid", "fuel_types", "fuel_price", "reviews", "historical_reviews", "review_sentiments",
]);

/** Shared schema for the list tools' paging controls. */
const LIST_LIMIT = z.number().int().min(1).max(500).optional()
  .describe(`Results per page (default ${DEFAULT_LIST_LIMIT}; plans cap at 20/50/100/500). Each page is charged its limit, so keep it small.`);
const MAX_RESULTS = z.number().int().min(1).max(MAX_RESULTS_CAP).optional()
  .describe(`Total results wanted (≤ ${MAX_RESULTS_CAP}): pages automatically at 'limit' per page and reports the summed api_calls_counted. Omit for one page.`);
...
export function registerTools(server: McpServer, getKey: () => string) {
  server.registerTool(
    "search_places",
    {
      title: "Search places",
      description:
        "Search the global Location Drive POI database by name or keyword — ranked by prominence for general terms " +
        "(e.g. 'hotel', 'coffee'), by similarity for specific names (typos and partial names still match). " +
        ...
        "Cost: limit API calls per page (default 20, charged whatever the page returns); max_results pages automatically " +
        "and the result's api_calls_counted is the summed total.",
      inputSchema: {
    ...
    async ({ query, country, category, limit, max_results }) => {
      try {
        return out(await ldList(getKey(), "/v1/places/search",
          { q: query, country, category, fields: SLIM_FIELDS }, { limit, max_results }));
  ...
  server.registerTool(
    "find_nearby",
    {
      ...
      inputSchema: {
        ...
        radius_m: z.number().int().min(50).max(500000).optional().describe("Search radius in meters (default 1000; plans cap at 5/25/100/500 km)"),
    ...
    async ({ latitude, longitude, radius_m, category, limit, max_results }) => {
      try {
        return out(await ldList(getKey(), "/v1/places/nearby",
          { lat: latitude, lng: longitude, radius: radius_m ?? 1000, category, fields: SLIM_FIELDS + ",distance_m" },
          { limit, max_results }));
      } catch (e) { return fail(e); }
    },
  );
  ...
  server.registerTool(
    "get_place_context",
    {
      ...
      description:
        ...
        "fields=reviews,historical_reviews. Cost: 1 API call.",
  ...
  server.registerTool(
    "get_review_history",
    {
      ...
      description:
        ...
        "instead; there is no city- or country-wide review search. Available on every plan. Cost: 1 API call.",
      inputSchema: { place_id: z.string().describe("Location Drive place ID") },
      annotations: READ_ONLY,
    },
    async ({ place_id }) => {
  ...
  server.registerTool(
    "scan_reviews",
    {
      ...
      description:
        ...
        "Cost: the nearby search is charged its limit (default 20) plus 1 API call per scanned place — up to 2 × limit API calls (≤ 80); " +
  ...
  server.registerTool(
    "get_hotel_rates",
    {
      ...
      description:
        ...
        "Available on every plan; when a hotel has no listed rates the response carries a rates_note. Cost: 1 API call.",
  ...
  server.registerTool(
    "get_building_polygon",
    {
      ...
      description:
        ...
        "Returns GeoJSON ready for maps. Available on every plan. Cost: 1 API call.",
      inputSchema: {
  ...
  server.registerTool(
    "find_building_at_point",
    {
      ...
      description:
        ...
        "and the business(es) inside. Location Drive's signature capability. Available on every plan. Cost: 1 API call.",
      inputSchema: {
        latitude: z.number().min(-90).max(90).describe("Latitude in decimal degrees (WGS84)"),
        longitude: z.number().min(-180).max(180).describe("Longitude in decimal degrees (WGS84)"),
      },
  ...
  server.registerTool(
    "brand_footprint",
    {
      ...
      description:
        ...
        "Use for 'how many <brand> locations are there', competitor analysis, or market sizing. Cost: 1 API call (summary endpoint). " +
        "For the individual stores use brand_locations.",
      inputSchema: {
  ...
  server.registerTool(
    "brand_locations",
    {
      ...
      description:
        "Every location of a brand — in one country or globally — as compact place records (address, coordinates, rating, " +
        "hours, contact) with the total count. Use for 'list the <brand> stores in <country>', store mapping, or coverage checks; " +
        "for counts by country use brand_footprint (1 API call). " +
        "Cost: limit API calls per page (default 20, charged whatever the page returns); max_results pages automatically " +
        "and the result's api_calls_counted is the summed total.",
      inputSchema: {
        brand: z.string().min(2).describe("Brand name, e.g. 'Starbucks'"),
        country: z.string().length(2).optional().describe("ISO-2 country filter, e.g. 'US'; omit for global"),
        limit: LIST_LIMIT,
        max_results: MAX_RESULTS,
      },
      annotations: READ_ONLY,
    },
    async ({ brand, country, limit, max_results }) => {
      try {
        return out(await ldList(getKey(), `/v1/brands/${encodeURIComponent(brand)}/locations`,
          { country, fields: SLIM_FIELDS }, { limit, max_results }));
      } catch (e) { return fail(e); }
    },
  );
  ...
  server.registerTool(
    "data_coverage",
    {
      ...
      description:
        ...
        "Call this first when unsure whether a region is covered. Cost: 1 API call.",
      inputSchema: {
